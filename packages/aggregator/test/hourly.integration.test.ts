...
import { HourlyReportFinalizer } from '../src/index.js';
...
describe('hourly report persistence', () => {
  it('persists hour_reports with minute_root and report_hash', () => {
    const tempDir = mkdtempSync(path.join(tmpdir(), 'fpho-hourly-'));
    tempPaths.push(tempDir);

    const dbPath = path.join(tempDir, 'hourly.sqlite');
    ...
    const result = finalizer.finalizeHour(1707346800);
    finalizer.close();
