...
describe('ingestor config and factory', () => {
  it('loads defaults from environment', () => {
    ...
    expect(config.rawTickRetentionSeconds).toBe(86_400);
    expect(config.rawTickPruneIntervalSeconds).toBe(3_600);
  ...
  it('parses explicit environment overrides', () => {
    const config = loadIngestorConfigFromEnv({
      ...
      FPHO_RAW_TICK_RETENTION_SECONDS: '0',
      FPHO_RAW_TICK_PRUNE_INTERVAL_SECONDS: '30',
    ...
    expect(config).toEqual({
      ...
      rawTickPruneIntervalSeconds: 30,
